...
export default function EditGigPage() {
  ...
  useEffect(() => {
    Promise.all([api.get(`/gigs/${id}`), api.get('/categories')])
      .then(([gigRes, catRes]) => {
      ...
      .catch(() => navigate('/dashboard', { state: { flash: { type: 'error', text: 'Usluga nije pronadjena.' } } }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.patch(`/gigs/${id}`, payload);
      navigate(`/gigs/${id}`, {
        state: { flash: { type: 'success', text: 'Usluga je uspjesno azurirana.' } },
      });
    } catch (err: unknown) {
      if (err && typeof err === 'object' && 'isAxiosError' in err) {
        setError(getApiErrorMessage(err, 'Greska pri azuriranju usluge.'));
      } else {
        setError(err instanceof Error ? err.message : 'Greska pri azuriranju usluge.');
      }
    } finally {
      setSaving(false);
    }
